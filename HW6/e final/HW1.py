...
# for i in range(100):
#     clf = Perceptron()
#     clf._allocate_parameter_mem(n_classes=3, n_features=2, coef_init=np.random.randn(3, 2),
#                                                        intercept_init=np.random.randn(3, ))
#     clf.fit(train_std2, lx2)
#     acc = clf.score(train_std2, lx2)
#     if acc > max_number:
#         max_number = acc
...
#                                                                intercept_init=np.random.randn(3, ))
#     clf.fit(train_std2, lx2)
#     acc = clf.score(test_train_std2, test_lx2)
#     if acc > max_number:
#         max_number = acc
#         max_weight = np.concatenate((clf.intercept_.reshape(3, 1), clf.coef_), axis=1)
#     print('\n')
...
for i in range(100):
    clf = Perceptron()
    clf. _allocate_parameter_mem(n_classes = 3, n_features = 2, coef_init = np.random. randn(3,2),intercept_init = np.random.randn(3,))
    clf.fit(train_std, l2)
