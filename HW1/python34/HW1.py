...
#这里处理要进行测试的文件
with open('synthetic1_test.csv') as test_data:
...
x_data, y_data, label_data, mean_data2 = read_and_mean('synthetic1_test.csv')

# #这里处理要进行测试的文件
# with open('synthetic2_train.csv') as train_data2:
#     reader2 = train_data2.readlines()   #一行一行读取要测试的文件 读出来好像是字符形式的？？？
#     train2 = [rows.split(',') for rows in reader2]   #按照逗号进行分割，数据一堆一堆分好
#     train_data2 = [data[:2] for data in train2]       #取出坐标存储
#     train_label2 = [data[2] for data in train2]       #取出标签存储
#     train_label2 = [int(test) for test in train_label2]       #将label转换为int类型，将换行符分开 #这里的test_table是已经分好的标签，等会要与这个标签进行比较
#
# for i in range(len(train_data2)):
#     train_data2[i] = [float(data) for data in train_data2[i]]
# output2 = judge(train_data2, mean_data)    # class_mean就等于mean_data 就是两个点
# print(output2)
# count2 = 0
# for i in range(len(train_data2)):
#     if output2[i] != train_label2[i]:
#         count2 = count2 + 1
# error2 = count2 / 100
# print(error2)
