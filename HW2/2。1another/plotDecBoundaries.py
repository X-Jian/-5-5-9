...
def plotDecBoundaries(training, label_train, sample_mean):
    ...
    plt.plot(training[label_train == 1, 0],training[label_train == 1, 1], 'rx')    #在数据中把标号为1的全挑出来，画散点  label_train == 1会返回下标？？？
    plt.plot(training[label_train == 2, 0],training[label_train == 2, 1], 'go')    #标签为2的x和y
    if nclass == 3:
